...
pub struct SocketClientBuilder<A> {
    pub(super) address: A,
}

impl<A> SocketClientBuilder<A> {
...
impl<A: Clone> Clone for SocketClientBuilder<A> {
    fn clone(&self) -> Self {
        SocketClientBuilder {
...
pub trait ServerBuild<K, V, C>: Sized {
...
impl<'a, K, V, C, B> ServerBuild<K, V, C> for B
where
    B: Build<C> + Sized,
